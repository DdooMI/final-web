...
export default function DesignerFormModal({ isOpen, onClose, designer = null, onSuccess }) {
    ...
    const [avatarPreview, setAvatarPreview] = useState(designer?.photoURL || '/person.gif');
    ...
    // Initialize form with designer data if editing
    useEffect(() => {
        if (designer) {
            setFormData({
                name: designer.name || '',
            ...
            // Reset form when adding a new designer
            setFormData({
                name: '',
                email: '',
                phone: '',
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (err.code === 'auth/email-already-in-use') {
                ...
                setError('Failed to save designer. Please try again.');
            }
        } finally {
            setLoading(false);
        }
                    ...
                    <div className="bg-white p-6">
                        <div className="flex justify-between items-center pb-3 border-b border-gray-200">
                            <h3 className="text-lg font-semibold text-gray-900">
                                {designer ? 'Edit Designer' : 'Add New Designer'}
                            </h3>
                            <button
                                onClick={onClose}
                        ...
                        <form onSubmit={handleSubmit} className="mt-4 space-y-4">
                            ...
                            <div>
                                ...
                                <input
                                    ...
                                    onChange={handleChange}
                                    disabled={designer !== null}
                                    className={`mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-[#C19A6B] focus:ring-[#C19A6B] ${designer ? 'bg-gray-100' : ''}`}
                                />
                                {designer && (
                                    <p className="mt-1 text-sm text-gray-500">Email cannot be changed once a designer is created.</p>
                                )}
                            </div>
                            {!designer && (
                                <div>
                      ...
                      file:bg-[#C19A6B] file:text-white hover:file:bg-[#A0784A]"
                            ...
                            <div className="pt-3 border-t flex justify-end space-x-3">
                                ...
                                >
                                    ...
                                    ) : designer ? 'Update Designer' : 'Add Designer'}
                                </button>
                            </div>
                        </form>
                    </div>
